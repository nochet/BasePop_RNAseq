...
sigCHS <- read.csv("../../processed/DESEQ/DEGs_CHS_0.05.csv")
names(sigCHS)[1]<-"FBgn_ID"
names(sigCHS)[3] <- "logFC"
names(sigCHS)[7] <- "adj.P.Val" 
sigCHS <- right_join(anotFbgn, sigCHS, by="FBgn_ID")


# export files for GO in DAVID
dfile <- select(sigInt, FBgn_ID)
names(dfile) <- NULL
efile <- select(sigCHS, FBgn_ID)
names(efile) <- NULL
...
# split Term column
anot <- separate(anot, col = Term, into = c("FBgn_ID", "Term"), sep = "\\~")

# change multiple columns to factor
names <- c('Category','FBgn_ID','Term','Genes')
anot[, names] <- lapply(anot[, names], factor)

anot <- select(anot, Category, FBgn_ID, Term, Genes, Benjamini)
...
sigCHS <- select(sigCHS, FBgn_ID, logFC)

circ <- circle_dat(anot, sigGenes)
circa <- circle_dat(anot, sigCHS)
...
xb <- inner_join(degs_treat,det_life, by="FBgn_ID")

# Search GO terms for these in the gene_association.fb
xc <- inner_join(xb, gene.assoc, by="FBgn_ID")
xc <- xc[order(xc$GO_Category), ]
xd <- subset(xc, xc$GO_Category=="P")
xdu <- unique(xd$FBgn_ID)
xe <- inner_join(xb, anotFbgn, by="FBgn_ID")
```
