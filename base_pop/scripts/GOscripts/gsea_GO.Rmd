...
# sigGeneSet(out.list[[ii]], cutoff = 0.1,heatmap = TRUE,
...
pqval01 <- subset(enrich.path,enrich.path$q.val<0.1)
pqval01 <- pqval01[order(pqval01$stat.mean, decreasing = TRUE),]
unq01 <- unique(pqval01$pathway)

ggplot(pqval01, aes(x=contrast, y=stat.mean, col=q.val)) +
  geom_point(stat = "identity") + ylab("Magnitude of gene-set level changes") +
  xlab("Diet contrast relative C diet") 

ggplot(pqval01, aes(x=pathway, y=stat.mean, col=q.val, label = contrast)) +
  geom_point() + ylab("Magnitude of geneset level changes") +
  xlab("Diet contrast relative C except DRHS") + 
  geom_text(aes(label = contrast), check_overlap = TRUE) +
