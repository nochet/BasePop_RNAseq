...
DR_O <- out.list[[1]][[2]]%>%as.data.frame()%>%add_column(contrast = "DR_O")
DR_O <- rownames_to_column(DR_O, "pathway")%>%
  dplyr::select(pathway,stat.mean,contrast)
DR_H <- out.list[[1]][[3]]%>%as.data.frame()%>%add_column(contrast = "DR_H")
DR_H <- rownames_to_column(DR_H, "pathway")%>%
  dplyr::select(pathway,stat.mean,contrast)

HS_B <- out.list[[2]][[1]]%>%as.data.frame()%>%add_column(contrast = "HS_B")
HS_B <- rownames_to_column(HS_B, "pathway")%>%
  dplyr::select(pathway,stat.mean,contrast)
HS_O <- out.list[[2]][[2]]%>%as.data.frame()%>%add_column(contrast = "HS_O")
HS_O <- rownames_to_column(HS_O, "pathway")%>%
  dplyr::select(pathway,stat.mean,contrast)
HS_H <- out.list[[2]][[3]]%>%as.data.frame()%>%add_column(contrast = "HS_H")
