...
# log2 transform expr values, rename columns
gfpkm_all <- log2(gfpkm_all+1)
colnames(gfpkm_all) <- phenDat$id

# Extract row names as a column
gfpkm_all <- cbind(GeneName = rownames(gfpkm_all), gfpkm_all)
rownames(gfpkm_all) <- NULL

# Write results to csv
write.csv(gfpkm_all, 
          "../processed/results/ballG_all_results/log2_gfpkm_all.csv", row.names=FALSE)
```
