...
sigInt <- merge(as.data.frame(anotFbgn), 
                       as.data.frame(sigInt), 
                       by="ID", sort=FALSE)
...
#sigGenes <- subset(sigGenes, sigGenes$logFC>=2.5)
...
# binary matrix
chord <- chord_dat(circ, sigInt, eproc)

# Generate the matrix with a list of selected genes
chord <- chord_dat(data = circ, genes = sigInt)
...
```{r}
l1 <- subset(circ, term == 'oxidation-reduction process', c(sigInt,logFC))
l2 <- subset(circ, term == 'carbohydrate metabolic process', c(sigInt,logFC))
l3 <- subset(circ, term == 'glucose import', c(sigInt,logFC))
