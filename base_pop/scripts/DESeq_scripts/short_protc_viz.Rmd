...
ll
ggsave(last_plot(),file = "../../plots/PCAplot_rlog_uncorrected_trt.pdf",width = 3,height=3)
...
gg <- ggplot(bcor.pca, aes(PC1, PC2, color=treatment, shape=tissue)) + 
  ...
  my_theme +
  theme_half_open() +
  scale_color_viridis(alpha = 1,option = "cividis") +
  #tx_fill_map() +
  labs(color="Treatment", shape="Tissue") 
#gg
gg <- gg + theme(
...
ggsave(gg,file = "../../plots/PCAplot_rlog_corrected_trt.pdf",width = 3.8,height=2.5)
...
cir <- ggarrange(b2, p1, ll, p2, ncol = 2, nrow = 2, labels = c("a", "c", "d", "b"))

ggplot2::ggsave(cir,file = "../../plots/transcripts_stat.pdf",width = 8,height=6)
```
