...
# check fold changes with Stanley results for bodies?
# only include peaks for difference QTL?
...
gid <- gmtable[gmtable$gname %in% all.dr$id,]
all.dr <- merge(all.dr, gid[,c("gname","FBgn")],by.y="gname", by.x='id')

rna.dr <- all.fc.dat[,1:11]

mm <- rna.dr[all.dr$FBgn,]
cor(mm$FC_DR_B[-14], log2(all.dr$mean.dcq[-14]))
...
gg <- read.csv(file = "../../processed/DESEQ/DEG_QTL/all_diffExpr_underQTL.csv", header=TRUE, stringsAsFactors = FALSE, row.names=1)

gg <- subset(gg, QTL == "LinDiscrB")
nrow(gg[gg$effect=="treat",])
