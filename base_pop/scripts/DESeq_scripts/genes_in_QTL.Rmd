...
transQTL <- select(transQTL, id, R6pos, R6st)
...
subset(qtl_degs,qtl_degs$QTL=="Ilp5")
...
subset(qtl_degs_int,qtl_degs_int$QTL=="step")
...
gsum <- read_lines("http://flybase.org/cgi-bin/get_static_page.pl?file=bulkdata7.html&title=Current%20Release/automated_gene_summaries.tsv.gz",
                   skip = 1)%>%
  str_split(pattern = "\t", simplify = TRUE) %>% 
  as_tibble() 

temp <- grep("lifespan", gsum$V2)
gsum[temp[1],'V2']
