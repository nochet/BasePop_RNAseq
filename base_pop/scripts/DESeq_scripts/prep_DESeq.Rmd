...
```{r}
# Just raise the maxit value on all trancripts with >0 reads

ddsDE_filt <- estimateSizeFactors(dds.ttSS)
nc <- counts(ddsDE_filt, normalized=TRUE)
#filter <- rowSums(nc >= 10) >= 5
filter <- rowSums(nc > 0) 
ddsDE_filt <- ddsDE_filt[filter,]

# Increase the maximum iterations (maxit) instead of running DESeq()
# deseq() runs these functions in this order

dds <- estimateSizeFactors(ddsDE_filt)
dds <- estimateDispersions(dds)
dds <- nbinomWaldTest(dds, maxit=5000)

which(duplicated(dds))



# getting duplicated gene names! Why??
# this site suggests remove duplicates:  https://support.bioconductor.org/p/90980/
o <- order(dds$Amean, decreasing=TRUE)
dds1 <- dds[o,]
...
# 10/3; maxit=20000, 2 rows did not converge
# 10/4; maxit=10000, 2 rows did not converge
# 10/4; maxit=20000, 1 row did not converge

# remove the non-converging rows
