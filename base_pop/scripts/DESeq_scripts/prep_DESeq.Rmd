...
# pairwise comparisions (tissue effect)
...
```{r}
cdrSig_HB <- subset(ff1, padj < 0.05)
summary(cdrSig_HB)
sum(cdrSig_HB$padj < 0.05, na.rm = TRUE)
cdrSig_HB$Gene <- rownames(cdrSig_HB)
cdrSig_HB <- cdrSig_HB[, c(7,1:6)]
(cdrSig_HB <- cdrSig_HB[order(cdrSig_HB$padj), ])
#write.csv(cdrSig_HB, "../../processed/DESEQ/DEGs_CDR_0.05.csv", row.names = FALSE)
mcols(cdrSig_HB)$description
```
