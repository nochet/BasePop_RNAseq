...
vst.trt <- varianceStabilizingTransformation(dds_deseq.01,
...
# vst data
mat <- assay(vst.trt)
mat <- limma::removeBatchEffect(mat, batch = phenDat.sva$SV1, 
                                batch2 = phenDat.sva$batch)
assay(vst.trt) <- mat
print(plotPCA(vst.trt, intgroup=c("treatment", "tissue"), returnData=FALSE))

# rlog data
rmat <- assay(rlog.trt)
rmat <- limma::removeBatchEffect(rmat, batch = phenDat.sva$SV1, batch2 = phenDat.sva$batch)
assay(rlog.trt) <- rmat
print(plotPCA(rlog.trt, intgroup=c("treatment", "tissue"), returnData=TRUE))
...
write.table(geneModuleMembership, "../../processed/DESEQ/Coexpression/MM.txt", sep="\t")
write.table(geneTraitSignificance, "../../processed/DESEQ/Coexpression/MM.txt", sep="\t")
...
visGreen = exportNetworkToVisANT(modTOM,
  file = paste("../../processed/DESEQ/Coexpression/minMod30/VisANTInput-", 
               module, ".txt", sep=""),
  weighted = TRUE,
  threshold = 0,
